...
ERNO.renderers.CSS3D = function(cubelets, cube) {
	...
	cube.faces.forEach(function(face, i) {
		...
		faceLabel = cube[face.face].label = new THREE.CSS3DObject(document.createElement(
			'div'));
	...
	function getFaceLabelElements() {
		return Array.prototype.slice.call(renderer.domElement.querySelectorAll(
			'.faceLabel'));
	}
...
ERNO.renderers.CSS3DCubelet = (function() {
	...
	return function(cubelet) {
		...
		var transformMap = [
			...
			"rotateX(   0deg ) translateZ( " + faceSpacing +
			"px ) rotateZ(   0deg )",
			"rotateX(  90deg ) translateZ( " + faceSpacing +
			"px ) rotateZ(   0deg )",
			"rotateY(  90deg ) translateZ( " + faceSpacing +
			"px ) rotateZ(   0deg )",
			"rotateX( -90deg ) translateZ( " + faceSpacing +
			"px ) rotateZ(  90deg )",
			"rotateY( -90deg ) translateZ( " + faceSpacing +
			"px ) rotateZ( -90deg )",
			"rotateY( 180deg ) translateZ( " + faceSpacing +
			"px ) rotateZ( -90deg )",
		...
		cubelet.faces.forEach(function(face) {
			...
			face.element.classList.add('face' + ERNO.Direction.getNameById(face.id)
				.capitalize());
			...
			style.OTransform = style.MozTransform = style.WebkitTransform = style.transform =
				cssTransform;
...
ERNO.renderers.CSS3DCubelet.methods = (function() {
	...
	return {
		...
		getFaceElements: function(selector) {
			...
			return Array.prototype.slice.call(this.css3DObject.element.querySelectorAll(
				'.face' + selectorString));
		...
		showIntroverts: function() {
			...
			return function(onlyAxis, soft) {
				...
				if (onlyAxis) {
					...
					only = (Math.abs(Math.round(axis.x)) === 1) ? '.axisX' : (Math.round(
						Math.abs(axis.y)) === 1) ? '.axisY' : '.axisZ';
				}

				this.getFaceElements('.faceIntroverted' + (onlyAxis !== undefined ?
					only : "")).forEach(showItem);
		...
		setOpacity: function(opacityTarget, onComplete) {
			...
			if (opacityTarget !== this.opacity) {
				...
				var
					that = this,
					tweenDuration = (opacityTarget - this.opacity).absolute().scale(0, 1, 0,
						1000 * 0.2)

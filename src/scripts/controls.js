ERNO.Controls = (function() {
	...
	return function(object, camera, domElement) {
		...
		function touchstart(event) {
			...
			if (api.enabled && projector.getIntersection(camera, event.touches[0].pageX,
					event.touches[0].pageY) === null) {

				state = STATE.ROTATE;

				direction.multiplyScalar(0);
				getMouseProjectionOnBall(event.touches[0].pageX, event.touches[0].pageY,
					mouse);
				lastPosition.copy(mouse);
		...
		function touchmove(event) {
			...
			if (api.enabled) {
				...
				getMouseProjectionOnBall(event.changedTouches[0].pageX, event.changedTouches[
					0].pageY, mouse);

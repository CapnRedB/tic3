...
ERNO.Projector = (function() {
	...
	return function(cube, domElement) {
		...
		function unprojectVector(vector, camera) {
			...
			viewProjectionMatrix.multiplyMatrices(camera.matrixWorld,
		...
		api = {
			...
			getIntersection: function(camera, mouseX, mouseY,
				...
				if (ray.isIntersectionSphere(sphere) &&
					...
					if (optionalPlaneTarget) {
						...
						optionalPlaneTarget.setFromNormalAndCoplanarPoint(normal,
							intersection);
						//console.log(optionalPlaneTarget);
					}
			...
			getFaceNormalForIntersection: function(intersection) {
			...
			getIntersectionOnPlane: function(camera, mouseX, mouseY, plane,
				optionalTarget) {
			...
			getCubeletAtIntersection: (function() {
				...
				return function(intersection) {
					...
					tmp.copy(intersection).add(box.max)
						...
						.set(Math.min(tmp.x | 0, 2), Math.min(3 - tmp.y | 0, 2), Math.min(
							3 - tmp.z | 0, 2));

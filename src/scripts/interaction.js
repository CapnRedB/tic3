	...
	return function(cube, camera, domElement, dragSpeed, multiDrag) {
		...
		api.getIntersectionAt = (function() {
			...
			return function(x, y) {
				...
				if (projector.getIntersection(camera, x, y, intersection3D, plane3D) ===
					null) return null;

				return {
					cubelet: projector.getCubeletAtIntersection(intersection3D),
					face: plane3D.normal.x === 1 ? "RIGHT" : plane3D.normal.x === -1 ?
						"LEFT" : plane3D.normal.y === 1 ? "UP" : plane3D.normal.y === -1 ?
						"DOWN" : plane3D.normal.z === 1 ? "FRONT" : "BACK"
				}
		...
		function snapVectorToBasis(vector) {
			...
			var max = Math.max(Math.abs(vector.x), Math.abs(vector.y), Math.abs(
				vector.z));
		...
		api.update = function() {
			...
			if (api.enabled && api.active && x !== undefined && y != undefined && (
					mouseX !== x || mouseY !== y)) {
		...
		function onInteractStart(event) {
			...
			if (api.enabled && event.button !== 2) {
				...
				if (intersected = projector.getIntersection(camera, mouseX, mouseY,
						intersection, plane)) {
					...
					if (cube.isTweening() === 0) {
						...
						time = (typeof window !== 'undefined' && window.performance !==
		...
		function onInteractEnd(event) {
			...
			if (api.enabled && (x !== mouseY || y !== mouseY) && axisDefined) {
				...
				var targetAngle = Math.round(angle / Math.PI * 0.5 * 4.0) * Math.PI *
					0.5;

				var velocityOfInteraction = direction.length() / ((typeof window !==
					'undefined' && window.performance !== undefined && window.performance
					.now !== undefined ? window.performance.now() : Date.now()) - time);
		...
		domElement.addEventListener('mouseup', function(event) {
			...
			if (!axisDefined && Math.abs(Math.sqrt(((bx - ax) * (bx - ax)) + ((by -
					ay) * (by - ay)))) < 10 * (window.devicePixelratio || 1)) {
